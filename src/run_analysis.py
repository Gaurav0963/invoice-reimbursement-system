...
class InvoicePolicyComparator:
    ...
    # def extract_text_from_zip(self, zip_file_path: str)->str:
    #     """Extract text from PDFs inside a ZIP file."""
    #     extracted_texts = []
    ...
    #                     extracted_texts.append(self.clean_text(self.extract_text_from_pdf(file_path)))
    #     return extracted_texts

    # def process_claim(self, invoice_text_list: list, policy_text:str)->List:
    #     document = []
    #     for invoice_text in invoice_text_list:
    #         result = self.analyse_invoice_against_policy(invoice_text_data=invoice_text, policy_text_data=policy_text)
...
if __name__ == "__main__":
    comparator = InvoicePolicyComparator()
    # vector_db = InvoiceVectorStore()
 
    zip_path = "E:\\FastAPI-chatbot\\data\\Cab_Bills.zip"
    policy_path = "E:\\FastAPI-chatbot\\data\\Policy-Nov-2024.pdf"

    # policy_text = comparator.clean_text(comparator.extract_text_from_pdf(pdf_path=policy_path))
    # inv_txt = comparator.extract_text_from_zip(zip_file_path=zip_path)
    # docs = comparator.process_claim(invoice_text_list=inv_txt, policy_text=policy_text)

    decisions, _ = comparator.process_zip_and_analyse(zip_file_path=zip_path, policy_path=policy_path)
    file = []
